...
export function ParentHanziLearning({ child }: { child: Child }) {
  ...
  const [schoolTargets, setSchoolTargets] = useState<HanziCharacterResource[]>([]);
  ...
  const [masteredTextMessage, setMasteredTextMessage] = useState("");
  ...
  async function loadSchoolTargets(nextPage = schoolPage) {
    const result = await parentApi.hanziSchoolTargets(child.id, nextPage, schoolPageSize);
    setSchoolTargets(result.characters);
    setSchoolTargetCount(result.total);
  }
  ...
  useEffect(() => {
    void loadSchoolTargets().catch((reason) => setError(reason instanceof Error ? reason.message : "学校学习清单加载失败"));
  }, [child.id, schoolPage, schoolPageSize]);

  useEffect(() => {
    setBusy(true);
    void parentApi.hanziCharacters(child.id, { q: searchQuery, status: statusFilter, page, pageSize })
      .then((result) => {
  ...
  }, [child.id, page, pageSize, searchQuery, statusFilter]);
  ...
  async function addSchoolTargetsFromText(event: FormEvent) {
    ...
    try {
      ...
      const refreshed = await parentApi.hanziCharacters(child.id, { q: searchQuery, status: statusFilter, page, pageSize });
  ...
  async function importMasteredFromText(event: FormEvent) {
    event.preventDefault();
    if (!masteredText.trim()) return;
    setBusy(true);
    setError("");
    setMasteredTextMessage("");
    try {
      const result = await parentApi.importHanziMasteredFromText(child.id, masteredText);
      const refreshed = await parentApi.hanziCharacters(child.id, { q: searchQuery, status: statusFilter, page, pageSize });
      setCharacters(refreshed.characters);
      setTotal(refreshed.total);
      await loadSchoolTargets();
      setMasteredText("");
      const details = [
        result.importedCount ? `已标记 ${result.importedCount} 个汉字为已掌握` : "没有新增已掌握汉字",
        result.alreadyMasteredCharacters.length ? `原已掌握：${result.alreadyMasteredCharacters.join("、")}` : "",
        result.missingCharacters.length ? `字库未找到：${result.missingCharacters.join("、")}` : "",
      ].filter(Boolean);
      setMasteredTextMessage(details.join("；"));
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "已掌握汉字导入失败");
    } finally {
      setBusy(false);
    }
  }

  const settingsPanel = <Panel title="每日学习参数">
  ...
  const schoolPanel = <Panel title={`学校优先清单（${schoolTargetCount} 个）`}>
      <p className="admin-help">这里按加入顺序展示学校近期所学汉字。系统会优先安排尚未学习的内容，学过后继续进入复习计划。</p>
      ...
      <div className="school-priority-list school-priority-list--hanzi">
        {schoolTargets.map((item, index) => <article key={item.id}>
          <span className="school-priority-list__order">{(schoolPage - 1) * schoolPageSize + index + 1}</span>
          <div className="school-priority-list__hanzi">{item.character}</div>
          <div><strong>{item.character} · {item.internalPinyin}</strong><small>{item.words.join("、")} · {item.meaning}</small></div>
          <AudioButton label="试听" url={item.characterAudioUrl} fallbackText={`${item.character}，${item.words[0] ?? item.character}`} />
          <button type="button" className="danger-text" disabled={busy} onClick={() => void toggleSchoolTarget(item)}>移出</button>
        </article>)}
        {!schoolTargets.length && !loading ? <div className="empty-state">还没有学校优先汉字。可以在上方批量输入，或从基础字库中选择。</div> : null}
      </div>
      <Pagination className="admin-pagination" current={schoolPage} pageSize={schoolPageSize} total={schoolTargetCount} showSizeChanger={paginationSizeChanger} pageSizeOptions={[10, 20, 50, 100]} showTotal={(value) => `共 ${value} 个优先汉字`} onShowSizeChange={(_, size) => { setSchoolPage(1); setSchoolPageSize(size); }} onChange={(nextPage, nextPageSize) => { setSchoolPage(nextPage); if (nextPageSize !== schoolPageSize) setSchoolPageSize(nextPageSize); }} />
    </Panel>;

  const libraryPanel = <Panel title={`基础汉字库（${total} 个）`}>
      <p className="admin-help">汉字资源由超级后台统一维护。这里按孩子的学习进度优先展示，家长可以搜索和试听。</p>
      <form className="hanzi-library-search" onSubmit={search}><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="搜索汉字、拼音、含义或例句" /><select aria-label="学习状态" value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value as LearningStatusFilter); setPage(1); }}><option value="ALL">全部状态</option><option value="UNLEARNED">未学习</option><option value="LEARNING">正在学习</option><option value="MASTERED">已掌握</option></select><button type="submit">搜索</button>{searchQuery || statusFilter !== "ALL" ? <button type="button" onClick={() => { setSearchInput(""); setSearchQuery(""); setStatusFilter("ALL"); setPage(1); }}>清除</button> : null}</form>
      <form className="school-target-text-form mastered-import-form" onSubmit={importMasteredFromText}>
        <label htmlFor="mastered-hanzi-input">导入孩子已掌握的汉字</label>
        <textarea id="mastered-hanzi-input" value={masteredText} onChange={(event) => setMasteredText(event.target.value)} placeholder="输入学校已经学会的汉字，例如：春夏秋冬\n支持空格、逗号或分行" rows={2} maxLength={2_000} />
        <div className="school-target-text-form__footer"><small>导入后会直接标记为“已掌握”，不会再安排为新字。</small><button type="submit" className="primary-button" disabled={busy || !masteredText.trim()}>{busy ? "导入中…" : "导入已掌握"}</button></div>
        {masteredTextMessage ? <div className="admin-notice" role="status">{masteredTextMessage}</div> : null}
      </form>
      {error ? <Notice error>{error}</Notice> : null}
        ...
        {characters.map((item) => <article key={item.id}>
          ...
          <div>
            ...
            <div className="hanzi-library-progress">
              ...
              {item.progress ? <small>巩固阶段 {item.progress.reviewStage}/7{item.progress.nextReviewDate ? ` · 下次 ${item.progress.nextReviewDate.slice(0, 10)}` : ""}</small> : null}
            </div>
  ...
  return <div className="admin-stack learning-workspace">
    <Tabs
      ...
      items={[
        ...
        { key: "library", label: "基础字库", children: libraryPanel },
      ]}
    />
  </div>;
}
...
const DEFAULT_POEM_SETTINGS: PoemLearningSettings = { enabled: false, learningWeekdays: [2, 4], learningTaskStars: 2, reviewTaskStars: 2, newPoemsPerSession: 1, reviewDailyLimit: 5 };
...
export function ParentPoemLearning({ child }: { child: Child }) {
  ...
  const [schoolTargets, setSchoolTargets] = useState<PoemResource[]>([]);
  const [schoolTargetCount, setSchoolTargetCount] = useState(0);
  const [schoolPage, setSchoolPage] = useState(1);
  ...
  async function loadSchoolTargets(nextPage = schoolPage) {
    const result = await parentApi.poemSchoolTargets(child.id, nextPage, schoolPageSize);
    setSchoolTargets(result.poems);
    setSchoolTargetCount(result.total);
  }
  async function loadPoems(nextQuery = activeQuery, nextGrade = activeGrade, nextPage = page) {
    const result = await parentApi.poems(child.id, { q: nextQuery || undefined, grade: nextGrade || undefined, status: statusFilter, page: nextPage, pageSize });
    setPoems(result.poems);
    setTotal(result.total);
  }
  useEffect(() => { setError(""); setPage(1); setSchoolPage(1); setActiveQuery(""); setActiveGrade(0); void loadSettings().catch((reason) => setError(reason instanceof Error ? reason.message : "古诗学习配置加载失败")); }, [child.id]);
  useEffect(() => { void loadPoems().catch((reason) => setError(reason instanceof Error ? reason.message : "古诗库读取失败")); }, [child.id, page, pageSize, activeQuery, activeGrade, statusFilter]);
  useEffect(() => { void loadSchoolTargets().catch((reason) => setError(reason instanceof Error ? reason.message : "学校学习清单加载失败")); }, [child.id, schoolPage, schoolPageSize]);
  useEffect(() => () => { audioRef.current?.pause(); audioRef.current = null; }, [child.id]);
  useEffect(() => { setSelectedIds(new Set()); }, [child.id, page, activeQuery, activeGrade, statusFilter]);
  ...
  async function importMasteredFromText(event: FormEvent) {
    ...
    setMasteredTextMessage("");
    try {
      const result = await parentApi.importPoemMasteredFromText(child.id, masteredText);
      await loadPoems(activeQuery, activeGrade, page);
      await loadSchoolTargets();
      setMasteredText("");
      const details = [
        result.importedCount ? `已标记 ${result.importedCount} 首古诗为已掌握` : "没有新增已掌握古诗",
        result.alreadyMasteredTitles.length ? `原已掌握：${result.alreadyMasteredTitles.map((title) => `《${title}》`).join("、")}` : "",
        result.missingTitles.length ? `资源库未找到：${result.missingTitles.map((title) => `《${title}》`).join("、")}` : "",
      ].filter(Boolean);
      setMasteredTextMessage(details.join("；"));
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "已掌握古诗导入失败");
    } finally {
      setBusy(false);
    }
  }
  ...
  const settingsPanel = <Panel title="古诗学习参数"><form className="admin-form poem-settings-form" onSubmit={save}><label className="checkbox field-span"><input type="checkbox" checked={settings.enabled} onChange={(event) => setSettings({ ...settings, enabled: event.target.checked })} />开启古诗学习任务</label><div className="field-span"><span className="poem-settings-label">每周新诗学习日（可多选）</span><div className="poem-weekday-picker">{POEM_WEEKDAYS.map((weekday) => <button type="button" key={weekday.value} className={settings.learningWeekdays.includes(weekday.value) ? "active" : ""} onClick={() => toggleWeekday(weekday.value)}>{weekday.label}</button>)}</div></div><label>每次学习新诗<NumberField type="number" min={1} max={5} value={settings.newPoemsPerSession} onChange={(event) => setSettings({ ...settings, newPoemsPerSession: Number(event.target.value) })} /></label><label>每日复习上限<NumberField type="number" min={1} max={20} value={settings.reviewDailyLimit} onChange={(event) => setSettings({ ...settings, reviewDailyLimit: Number(event.target.value) })} /></label><label>学习任务星星<NumberField type="number" min={1} max={999} value={settings.learningTaskStars} onChange={(event) => setSettings({ ...settings, learningTaskStars: Number(event.target.value) })} /></label><label>复习任务星星<NumberField type="number" min={1} max={999} value={settings.reviewTaskStars} onChange={(event) => setSettings({ ...settings, reviewTaskStars: Number(event.target.value) })} /></label><div className="field-span admin-help">所选日期只控制“学习新古诗”；复习任务按孩子的实际回忆表现自动出现。顺利想起会逐步拉长间隔，提示后想起或忘记会更早再见。</div><div className="form-actions field-span"><button className="primary-button" disabled={busy}>{busy ? "保存中…" : "保存设置"}</button></div></form>{error ? <Notice error>{error}</Notice> : null}</Panel>;

  const schoolPanel = <Panel title={`学校优先清单（${schoolTargetCount} 首）`}><p className="admin-help">按加入顺序展示。未学古诗会优先从这里选择，已学内容仍按记忆曲线复习。</p>{error ? <Notice error>{error}</Notice> : null}<div className="school-priority-list">{schoolTargets.map((poem, index) => <article key={poem.id}><span className="school-priority-list__order">{(schoolPage - 1) * schoolPageSize + index + 1}</span>{poem.imageUrl ? <img src={poem.imageUrl} alt="" loading="lazy" /> : <div className="school-priority-list__placeholder">诗</div>}<div><strong>《{poem.title}》</strong><small>{poem.dynasty} · {poem.author} · {poem.grade} 年级</small></div><button type="button" className="ghost-button" disabled={!poem.audioUrl} onClick={() => toggleAudio(poem)}>{playingId === poem.id ? "停止" : "试听"}</button><button type="button" className="danger-text" disabled={busy} onClick={() => void toggleSchoolTarget(poem)}>移出</button></article>)}{!schoolTargets.length ? <div className="empty-state">还没有学校优先古诗。请到“古诗资源库”搜索并批量加入。</div> : null}</div><Pagination className="admin-pagination" current={schoolPage} pageSize={schoolPageSize} total={schoolTargetCount} showSizeChanger={paginationSizeChanger} pageSizeOptions={[10, 20, 50, 100]} showTotal={(value) => `共 ${value} 首优先古诗`} onShowSizeChange={(_, size) => { setSchoolPage(1); setSchoolPageSize(size); }} onChange={(nextPage, nextPageSize) => { setSchoolPage(nextPage); if (nextPageSize !== schoolPageSize) setSchoolPageSize(nextPageSize); }} /></Panel>;

  const libraryPanel = <Panel title={`古诗资源库（${total} 首）`}><p className="admin-help">资源由超级后台统一维护。家长可以查询、试听并加入学校优先清单。</p><form className="poem-library-toolbar" onSubmit={search}><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索标题、作者或诗句" /><select value={grade} onChange={(event) => setGrade(Number(event.target.value))}><option value={0}>全部年级</option>{[1, 2, 3, 4, 5, 6].map((value) => <option value={value} key={value}>{value} 年级</option>)}</select><select aria-label="学习状态" value={statusFilter} onChange={(event) => { setStatusFilter(event.target.value as LearningStatusFilter); setPage(1); }}><option value="ALL">全部状态</option><option value="UNLEARNED">未学习</option><option value="LEARNING">正在学习</option><option value="MASTERED">已掌握</option></select><button className="ghost-button" disabled={busy}>查询</button></form><form className="school-target-text-form mastered-import-form" onSubmit={importMasteredFromText}><label htmlFor="mastered-poem-input">导入孩子已掌握的古诗</label><textarea id="mastered-poem-input" value={masteredText} onChange={(event) => setMasteredText(event.target.value)} placeholder="输入学校已经学会的古诗标题，例如：静夜思\n每行一首，也支持逗号分隔" rows={2} maxLength={5_000} /><div className="school-target-text-form__footer"><small>导入后会直接标记为“已掌握”，不会再安排为新诗。</small><button type="submit" className="primary-button" disabled={busy || !masteredText.trim()}>{busy ? "导入中…" : "导入已掌握"}</button></div>{masteredTextMessage ? <div className="admin-notice" role="status">{masteredTextMessage}</div> : null}</form><div className="school-target-toolbar"><label><input type="checkbox" checked={poems.length > 0 && poems.every((poem) => selectedIds.has(poem.id))} onChange={toggleAllPoems} />全选本页（{poems.length} 首）</label><span>已选 {selectedIds.size} 首</span><button type="button" className="primary-button" disabled={busy || !poems.some((poem) => selectedIds.has(poem.id) && !poem.schoolTarget)} onClick={() => void batchUpdateSchoolTargets("add")}>批量加入学校学习</button><button type="button" className="ghost-button" disabled={busy || !poems.some((poem) => selectedIds.has(poem.id) && poem.schoolTarget)} onClick={() => void batchUpdateSchoolTargets("remove")}>批量移出学校清单</button></div>{error ? <Notice error>{error}</Notice> : null}<div className="table-wrap poem-library-table responsive-card-table"><table><thead><tr><th>选择</th><th>年级</th><th>古诗</th><th>作者</th><th>学习状态</th><th>媒体</th><th>学校学习</th></tr></thead><tbody>{poems.map((poem) => <tr key={poem.id}><td data-label="选择"><input type="checkbox" aria-label={`选择《${poem.title}》`} checked={selectedIds.has(poem.id)} onChange={() => toggleSelected(poem.id)} /></td><td data-label="年级">{poem.grade} 年级{poem.semester}</td><td data-label="古诗"><strong>《{poem.title}》</strong><small>{poem.content}</small></td><td data-label="作者">{poem.dynasty} · {poem.author}</td><td data-label="学习状态"><span className={`status status--${poem.progress?.status === "MASTERED" ? "completed" : poem.progress ? "pending" : "cancelled"}`}>{poem.progress?.status === "MASTERED" ? "已掌握" : poem.progress ? `巩固阶段 ${poem.progress.reviewStage}/7` : "未学习"}</span>{poem.progress?.nextReviewDate ? <small>下次 {poem.progress.nextReviewDate.slice(0, 10)}</small> : null}</td><td data-label="媒体"><div className="poem-media-cell">{poem.imageUrl ? <img src={poem.imageUrl} alt={`《${poem.title}》配图`} loading="lazy" decoding="async" /> : <div className="poem-media-placeholder">暂无配图</div>}<div className="poem-media-actions"><button type="button" className="hanzi-audio-button" disabled={!poem.audioUrl} onClick={() => toggleAudio(poem)}>{playingId === poem.id ? "停止" : "试听朗读"}</button></div></div></td><td data-label="学校学习"><button type="button" className={poem.schoolTarget ? "ghost-button" : "primary-button"} disabled={busy} onClick={() => void toggleSchoolTarget(poem)}>{poem.schoolTarget ? "移出清单" : "加入学校学习"}</button></td></tr>)}</tbody></table>{!poems.length ? <div className="empty-state">没有找到符合条件的古诗</div> : null}</div><Pagination className="admin-pagination" current={page} pageSize={pageSize} total={total} showSizeChanger={paginationSizeChanger} pageSizeOptions={[10, 20, 50, 100]} showTotal={(value) => `共 ${value} 首古诗`} onShowSizeChange={(_, size) => { setPage(1); setPageSize(size); }} onChange={(nextPage, nextPageSize) => { setPage(nextPage); if (nextPageSize !== pageSize) setPageSize(nextPageSize); }} /></Panel>;
