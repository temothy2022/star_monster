...
type Section = "metrics" | "performance" | "ai-usage" | "families" | "resources" | "pet-growth" | "mascot-assets" | "ai-settings" | "platform-features" | "audit";
...
function FamilyCard({
  ...
  async function showLoginCode(child: Family["children"][number]) {
    const result = await adminApi.childLoginCode(child.id);
    if (result.loginCode) {
      onCode(`${child.nickname ?? "孩子"}的探险代码`, result.loginCode);
      return;
    }
    if (!window.confirm(`这个旧账号目前只能确认代码尾号 ${result.loginCodeLastFour}。重新生成后即可随时查看完整代码，旧代码会立即失效。继续吗？`)) return;
    const regenerated = await adminApi.regenerateCode(child.id);
    onCode(`${child.nickname ?? "孩子"}的新登录代码`, regenerated.loginCode);
    onChanged();
  }
  return <article className="super-family-card"><header><div><h3>{family.name}</h3><p>创建于 {formatDate(family.createdAt)} · {family.users.length} 位家长 · {family.children.length} 个孩子</p></div><div className="super-account__actions"><span className={`status status--${family.status === "ACTIVE" ? "completed" : "cancelled"}`}>{family.status === "ACTIVE" ? "正常" : "已停用"}</span><button className="primary-button" onClick={onViewData}>查看数据</button><button onClick={() => { const name = window.prompt("修改家庭名称", family.name); if (name?.trim()) void adminApi.updateFamily(family.id, { name: name.trim() }).then(onChanged); }}>改名</button><button onClick={() => void adminApi.updateFamily(family.id, { status: family.status === "ACTIVE" ? "DISABLED" : "ACTIVE" }).then(onChanged)}>{family.status === "ACTIVE" ? "停用家庭" : "恢复家庭"}</button><button onClick={createParent}>添加家长</button><button className="primary-button" onClick={createChild}>添加孩子</button></div></header><div className="super-family-card__body"><section><h4>家长账号</h4><div className="super-account-list">{family.users.map((parent) => <div className="super-account" key={parent.id}><div><strong>{parent.displayName}</strong><p>{parent.username} · 最后活跃 {formatLastActive(parent.lastActiveAt)}</p></div><div className="super-account__actions"><button onClick={() => { const name = window.prompt("修改家长显示名称", parent.displayName); if (name?.trim()) void adminApi.updateUser(parent.id, { displayName: name.trim() }).then(onChanged); }}>编辑</button><button onClick={() => void adminApi.updateUser(parent.id, { status: parent.status === "ACTIVE" ? "DISABLED" : "ACTIVE" }).then(onChanged)}>{parent.status === "ACTIVE" ? "停用" : "启用"}</button><button onClick={() => { const password = window.prompt("输入新的家长密码（至少 8 位），确认后现有设备会退出"); if (password && password.length >= 8) void adminApi.resetPassword(parent.id, password).then(() => window.alert("密码已重置")); }}>重置密码</button></div></div>)}{!family.users.length && <div className="empty-state">没有家长账号</div>}</div></section><section><h4>孩子账号</h4><div className="super-account-list">{family.children.map((child) => <div className="super-account" key={child.id}><div><strong>{child.nickname ?? "尚未设置昵称"}</strong><p>代码尾号 {child.loginCodeLastFour} · {child.petType ?? "未选星宠"} · 最后活跃 {formatLastActive(child.lastActiveAt)}</p></div><div className="super-account__actions"><button onClick={() => void showLoginCode(child)}>查看完整代码</button><button onClick={() => { const nickname = window.prompt("修改孩子昵称（2–9 个字符）", child.nickname ?? ""); if (nickname?.trim() && nickname.trim().length >= 2 && nickname.trim().length <= 9) void adminApi.updateChild(child.id, nickname.trim()).then(onChanged); }}>编辑</button><button onClick={() => void adminApi.setChildStatus(child.id, child.status === "ACTIVE" ? "DISABLED" : "ACTIVE").then(onChanged)}>{child.status === "ACTIVE" ? "停用" : "启用"}</button><button onClick={() => window.confirm("重新生成后旧登录代码立即失效，但已登录设备不会退出。继续吗？") && void adminApi.regenerateCode(child.id).then((result) => { onCode(`${child.nickname ?? "孩子"}的新登录代码`, result.loginCode); onChanged(); })}>重置代码</button><button onClick={() => window.confirm("让这个孩子的所有设备退出登录？") && void adminApi.logoutChild(child.id).then(() => window.alert("所有设备已退出"))}>设备全退</button></div></div>)}{!family.children.length && <div className="empty-state">没有孩子账号</div>}</div></section></div></article>;
}

const SUPER_CATEGORY_LABELS: Record<string, string> = { MATH: "数学", EXERCISE: "运动", CHORES: "生活习惯", CHINESE: "语文", ENGLISH: "英语", OTHER: "综合任务" };
const SUPER_WISH_CATEGORY_LABELS: Record<string, string> = { SPORTS: "运动", TELEVISION: "娱乐时间", TOYS: "物品消费", GAMES: "娱乐时间" };
const SUPER_STATUS_LABELS: Record<string, string> = { PENDING: "待处理", ARRANGED: "已安排", COMPLETED: "已完成", CANCELLED: "已取消" };

function FamilyOverview({ overview, onBack }: { overview: FamilyDataOverview; onBack: () => void }) {
  const totalTasks = overview.children.reduce((sum, child) => sum + child.taskStats.periodTotal, 0);
  const completedTasks = overview.children.reduce((sum, child) => sum + child.taskStats.periodCompleted, 0);
  const totalRedemptions = overview.children.reduce((sum, child) => sum + child.redemptions.length, 0);
  return <div className="admin-stack super-family-overview"><div className="super-detail-toolbar"><button className="ghost-button" type="button" onClick={onBack}>返回家庭列表</button><div><h2>{overview.family.name} · 数据概览</h2><p>统计范围：{overview.from.slice(0, 10)} 至 {overview.to.slice(0, 10)}，任务配置展示当前全部内容</p></div></div><div className="metric-grid"><article><span>孩子</span><strong>{overview.children.length}</strong><small>家庭成员</small></article><article><span>近 30 天任务完成</span><strong>{completedTasks}/{totalTasks}</strong><small>{totalTasks ? Math.round(completedTasks / totalTasks * 100) : 0}% 完成率</small></article><article><span>星星余额</span><strong>{overview.children.reduce((sum, child) => sum + child.starBalance, 0)}</strong><small>所有孩子合计</small></article><article><span>近 30 天兑换</span><strong>{totalRedemptions}</strong><small>包含已完成和已取消</small></article></div>{overview.children.map((child) => <section className="super-family-overview__child" key={child.id}><header><div><h3>{child.nickname ?? "尚未设置昵称"}</h3><p>{child.petType ?? "未选择星宠"} · 最后活跃 {formatLastActive(child.lastActiveAt)}</p></div><div className="super-family-overview__balances"><strong>★ {child.starBalance}</strong><span>累计获得 {child.lifetimeStarsEarned} 星</span></div></header><div className="super-family-overview__stats"><div><span>今日任务</span><strong>{child.taskStats.todayCompleted}/{child.taskStats.todayTotal}</strong></div><div><span>近 30 天完成率</span><strong>{child.taskStats.completionRate === null ? "—" : `${child.taskStats.completionRate}%`}</strong></div><div><span>近 30 天得星</span><strong>+{child.starStats.periodEarned}</strong></div><div><span>近 30 天消费</span><strong>-{child.starStats.periodSpent}</strong></div><div><span>完成尝试</span><strong>{child.taskStats.attemptsCompleted}</strong></div><div><span>放弃尝试</span><strong>{child.taskStats.attemptsAbandoned}</strong></div></div><div className="super-family-overview__columns"><div><h4>任务配置（{child.taskTemplates.length}）</h4><div className="super-data-table"><table><thead><tr><th>任务</th><th>分类</th><th>周期</th><th>星星</th><th>状态</th></tr></thead><tbody>{child.taskTemplates.map((task) => <tr key={task.id}><td>{task.title}</td><td>{SUPER_CATEGORY_LABELS[task.category] ?? task.category}</td><td>{task.scheduleKind === "DAILY" ? "每天" : task.scheduleKind === "WORKDAYS" ? "工作日" : task.scheduleKind === "SELECTED_WEEKDAYS" ? "指定星期" : "一次性"}</td><td>★ {task.baseStars}</td><td>{task.isEnabled ? "启用" : "停用"}</td></tr>)}</tbody></table>{!child.taskTemplates.length && <div className="empty-state">暂无任务配置</div>}</div></div><div><h4>星愿配置（{child.wishes.length}）</h4><div className="super-data-table"><table><thead><tr><th>星愿</th><th>分类</th><th>价格</th><th>兑换次数</th><th>状态</th></tr></thead><tbody>{child.wishes.map((wish) => <tr key={wish.id}><td>{wish.title}</td><td>{SUPER_WISH_CATEGORY_LABELS[wish.category] ?? wish.category}</td><td>★ {wish.costStars}</td><td>{wish.redemptionCount}</td><td>{wish.isEnabled ? "启用" : "停用"}</td></tr>)}</tbody></table>{!child.wishes.length && <div className="empty-state">暂无星愿配置</div>}</div></div></div><div className="super-family-overview__columns"><div><h4>最近兑换记录</h4><div className="super-data-table"><table><thead><tr><th>星愿</th><th>星星</th><th>状态</th><th>时间</th></tr></thead><tbody>{child.redemptions.map((item) => <tr key={item.id}><td>{item.titleSnapshot}</td><td>★ {item.costStarsSnapshot}</td><td>{SUPER_STATUS_LABELS[item.status] ?? item.status}</td><td>{formatDate(item.requestedAt)}</td></tr>)}</tbody></table>{!child.redemptions.length && <div className="empty-state">暂无兑换记录</div>}</div></div><div><h4>星星流水</h4><div className="super-data-table"><table><thead><tr><th>类型</th><th>变化</th><th>余额</th><th>时间</th></tr></thead><tbody>{child.ledger.slice(0, 20).map((item) => <tr key={item.id}><td>{item.reason ?? item.type}</td><td className={item.amount >= 0 ? "positive" : "negative"}>{item.amount >= 0 ? "+" : ""}{item.amount}</td><td>{item.balanceAfter}</td><td>{formatDate(item.createdAt)}</td></tr>)}</tbody></table>{!child.ledger.length && <div className="empty-state">暂无星星流水</div>}</div></div></div></section>)}</div>;
}
...
function FamiliesView() {
  ...
  async function viewOverview(familyId: string) {
    ...
    try { setOverview(await adminApi.familyOverview(familyId)); }
    catch (reason) { window.alert(reason instanceof Error ? reason.message : "家庭数据读取失败"); }
...
export function App() {
  ...
  return <div className="admin-app super-app"><aside className="admin-sidebar"><div className="admin-brand"><span>★</span><div><strong>星宠成长基地</strong><small>超级管理后台</small></div></div><nav><button className={section === "metrics" ? "active" : ""} onClick={() => setSection("metrics")}><span>▦</span>运营概览</button><button className={section === "performance" ? "active" : ""} onClick={() => setSection("performance")}><span>◷</span>性能诊断</button><button className={section === "ai-usage" ? "active" : ""} onClick={() => setSection("ai-usage")}><span>◒</span>模型调用</button><button className={section === "families" ? "active" : ""} onClick={() => setSection("families")}><span>♟</span>家庭与账号</button><button className={section === "resources" ? "active" : ""} onClick={() => setSection("resources")}><span>✎</span>学习资源</button><button className={section === "pet-growth" ? "active" : ""} onClick={() => setSection("pet-growth")}><span>✦</span>星宠成长</button><button className={section === "mascot-assets" ? "active" : ""} onClick={() => setSection("mascot-assets")}><span>◉</span>星宠素材</button><button className={section === "ai-settings" ? "active" : ""} onClick={() => setSection("ai-settings")}><span>AI</span>AI 平台配置</button><button className={section === "platform-features" ? "active" : ""} onClick={() => setSection("platform-features")}><span>⚙</span>平台功能</button><button className={section === "audit" ? "active" : ""} onClick={() => setSection("audit")}><span>≡</span>审计日志</button></nav><div className="admin-sidebar__account"><div><strong>{user.displayName}</strong><small>{user.username}</small></div><button onClick={() => void staffApi.logout().then(() => setUser(null))}>退出</button></div></aside><main className="admin-main"><header className="admin-topbar"><div><p>超级后台 / {labels[section]}</p><h1>{labels[section]}</h1></div><div className="topbar-balance"><span>系统状态</span><strong>● 正常</strong></div></header><div className="admin-content">{error && <div className="admin-notice admin-notice--error" onClick={() => setError("")}>{error} · 点击关闭</div>}{section === "metrics" && <MetricsView />}{section === "performance" && <PerformanceMonitoring />}{section === "ai-usage" && <AiUsageMonitoring />}{section === "families" && <FamiliesView />}{section === "resources" && <LearningResources />}{section === "pet-growth" && <PetGrowthManagement />}{section === "mascot-assets" && <MascotAssetManagement />}{section === "ai-settings" && <AiPlatformSettings />}{section === "platform-features" && <PlatformFeatureSettings />}{section === "audit" && <AuditView />}</div></main></div>;
}
