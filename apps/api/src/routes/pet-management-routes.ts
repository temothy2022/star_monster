...
const parentSettingsSchema = z.object({
  ...
  satietyDecayMinutes: z.number().int().min(10).max(10080).optional(),
  hydrationDecayMinutes: z.number().int().min(10).max(10080).optional(),
  dailyWasteCount: z.number().int().min(0).max(8).optional(),
  wasteCleanCostStars: z.number().int().min(0).max(100).optional(),
  ...
  app.patch("/api/parent/children/:id/pet-growth/settings", async (request, reply) => {
    ...
    return {
      settings: {
        ...
        satietyDecayMinutes: profile.satietyDecayMinutes,
        hydrationDecayMinutes: profile.hydrationDecayMinutes,
        dailyWasteCount: profile.dailyWasteCount,
        wasteCleanCostStars: profile.wasteCleanCostStars,
