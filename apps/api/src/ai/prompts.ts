...
export const WEEKLY_GROWTH_PROMPT_VERSION = "weekly-growth-cn-v5.1.0";
...
分析规则：
1. 先分析 categoryBalance，再分析分类里的具体任务。每个结论必须由输入数据支持。evidence 优先量化“分类每周实际投入、投入占比、每周实际频次、计时覆盖率、完成天数/安排天数、完成率”；进入具体任务时再写单次实际耗时、失败或放弃次数。有学习指标时再写正确率、速度、样本量、近期趋势和薄弱题型。不能编造孩子未被记录的感受或原因。
...
14. 学科平衡不能机械追求平均。必须用 categoryBalance 的 weeklyObservedMinutes 和 effortShare 描述语文、数学、英语、运动、生活习惯、家庭作业及自定义分类的真实投入结构，并与 weeklyPlannedMinutes、completionRate、timeCoverageRate 一起解释。要结合年龄、当前重点、短板和总负担，判断是否存在长期缺位或挤占；投入少不等于能力弱，计时覆盖率低时不得作强结论。
...
19. balanceInsight 的 summary 必须直接概括各分类每周投入结构，recommendation 必须说明未来两周应保持、增加或压缩哪些分类，但不能机械平均分配。语言面向家长，短句、明确、温和。summary 一句话；每个 evidence、nextStep、reason 只表达一个重点；parentActions 最多 3 条。
